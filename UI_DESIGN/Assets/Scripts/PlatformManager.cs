...
public class PlatformManager : PlatformGenericSingleTon<PlatformManager>
{
    ...
    void OnActiveSceneChanged(Button b) {


        if (b.name == "Programming") {
         ...
         if (SceneManager.GetActiveScene().name == "Programming") {
                ...
                if (Input.GetMouseButtonUp(0))
                {
                    ...
                    if (hit)
                    {
                        ...
                        {
                            PlatformDataNode pdn = clickedGameObject.transform.GetComponent<PlatformDataNode>();
                            pdn.ResetDataNode();
                        }

                        clickedGameObject = hitInfo.transform.gameObject;
                        PlatformDataNode newPdn = clickedGameObject.transform.GetComponent<PlatformDataNode>();
                        newPdn.SelectNode();
    ...
    public void BuildPlatform(PlatformConfigurationData pcd)
    {
        ...
        for (int i = 0; i < X; i += 1)
        {
            for (int j = 0; j < Y; j += 1)
            {
                ...
                cube[i, j].transform.position = new Vector3(i + i * SPACE, 0, j + j * SPACE);
